...
# mDNS Service Discovery Configuration (for MCP protocol only)
...
# Database path (use SQLite-vec by default)
if STORAGE_BACKEND == 'sqlite_vec' and SQLITE_VEC_PATH:
    DATABASE_PATH = SQLITE_VEC_PATH
else:
    # Fallback to a default SQLite-vec path
    DATABASE_PATH = os.path.join(BASE_DIR, 'memory.db')
